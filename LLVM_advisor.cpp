...
namespace{
	...
	static cl::opt<std::string> FName(
		"fname",
		cl::desc("Function name to match"),
		cl::value_desc("name"),
		cl::init("")
	);

	static cl::opt<std::string> LineRangeStr(
		"line_range",
		cl::desc("Line range in format start-end (e.g. 10-20)"),
		cl::value_desc("range"),
		cl::init("")
	);

	struct device_pass : public ModulePass{
        static char ID;   
        device_pass() : ModulePass(ID) {}
        ...
        virtual bool runOnModule(Module &M)
        {
			...
			if (!LineRangeStr.empty()) {
				std::stringstream ss(LineRangeStr);
				std::string token;
				if (std::getline(ss, token, '-')) {
					lineStart = std::stoi(token);
					if (std::getline(ss, token, '-')) {
						lineEnd = std::stoi(token);
					}
				}
			}

			// TODO: Checks on line_range and fname
			// errs() << "fname = " << FName << ", line range = " << lineStart << " to " << lineEnd << "\n";
			

			LLVMContext &Ctx = M.getContext();

			FunctionCallee recordLoadFunc = M.getOrInsertFunction("recordLoad",
				Type::getVoidTy(Ctx), Type::getInt32Ty(Ctx), Type::getInt32Ty(Ctx));
			FunctionCallee recordStoreFunc = M.getOrInsertFunction("recordStore",
				Type::getVoidTy(Ctx), Type::getInt32Ty(Ctx), Type::getInt32Ty(Ctx));
			FunctionCallee recordStartFunc = M.getOrInsertFunction("recordStartTime",
				Type::getVoidTy(Ctx), Type::getInt32Ty(Ctx));
			FunctionCallee recordEndFunc = M.getOrInsertFunction("recordEndTime",
				Type::getVoidTy(Ctx), Type::getInt32Ty(Ctx));

			for(Module::iterator F = M.begin(), E = M.end(); F!= E; ++F)
            {
				errs() << F->getName().str() << "\n";
				std::string fun_name_str =  F->getName().str();

				if (fun_name_str.find(fname) != std::string::npos){ 

					for(Function::iterator BB = F->begin(), E = F->end(); BB != E; ++BB)
					{
						device_pass::runOnBasicBlock(BB. lineStart, lineEnd);
					}
				}
            }
			...
			return false;
				...
				// Generate flat thread ID for 3D CUDA kernel
				Value *getFlatThreadId(IRBuilder<> &Builder, Module &M) {
					LLVMContext &Ctx = M.getContext();
			
					auto get = [&](Intrinsic::ID id) {
						return Builder.CreateCall(Intrinsic::getDeclaration(&M, id));
					};
			
					Value *tid_x = get(Intrinsic::nvvm_read_ptx_sreg_tid_x);
					Value *tid_y = get(Intrinsic::nvvm_read_ptx_sreg_tid_y);
					Value *tid_z = get(Intrinsic::nvvm_read_ptx_sreg_tid_z);
			
					Value *ntid_x = get(Intrinsic::nvvm_read_ptx_sreg_ntid_x);
					Value *ntid_y = get(Intrinsic::nvvm_read_ptx_sreg_ntid_y);
					Value *ntid_z = get(Intrinsic::nvvm_read_ptx_sreg_ntid_z);
			
					Value *ctaid_x = get(Intrinsic::nvvm_read_ptx_sreg_ctaid_x);
					Value *ctaid_y = get(Intrinsic::nvvm_read_ptx_sreg_ctaid_y);
					Value *ctaid_z = get(Intrinsic::nvvm_read_ptx_sreg_ctaid_z);
			
					Value *nctaid_x = get(Intrinsic::nvvm_read_ptx_sreg_nctaid_x);
					Value *nctaid_y = get(Intrinsic::nvvm_read_ptx_sreg_nctaid_y);
					Value *nctaid_z = get(Intrinsic::nvvm_read_ptx_sreg_nctaid_z);
			
					// blockDim.x * threadIdx.y
					Value *tmp1 = Builder.CreateMul(tid_y, ntid_x);
					// + threadIdx.x
					Value *offset_in_block = Builder.CreateAdd(tmp1, tid_x);
					// blockDim.x * blockDim.y * threadIdx.z
					Value *tmp2 = Builder.CreateMul(tid_z, Builder.CreateMul(ntid_x, ntid_y));
					offset_in_block = Builder.CreateAdd(offset_in_block, tmp2);
			
					// gridDim.x * blockIdx.y
					Value *tmp3 = Builder.CreateMul(ctaid_y, nctaid_x);
					// + blockIdx.x
					Value *grid_idx_flat = Builder.CreateAdd(tmp3, ctaid_x);
					// gridDim.x * gridDim.y * blockIdx.z
					Value *tmp4 = Builder.CreateMul(ctaid_z, Builder.CreateMul(nctaid_x, nctaid_y));
					grid_idx_flat = Builder.CreateAdd(grid_idx_flat, tmp4);
			
					// blockDim.x * blockDim.y * blockDim.z
					Value *block_size = Builder.CreateMul(ntid_x, Builder.CreateMul(ntid_y, ntid_z));
					Value *block_offset = Builder.CreateMul(grid_idx_flat, block_size);
			
					return Builder.CreateAdd(block_offset, offset_in_block);
				}
			
		
    	virtual bool runOnBasicBlock(Function::iterator &BB, int lineStart, int lineEnd){
	    	...
	    	for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI){
				...
				if (Line == lineStart)
					Builder.CreateCall(recordStartFunc, {tid});
				
				if (Line == lineEnd)
					Builder.CreateCall(recordEndTime);

				
				Value *tid = getFlatThreadId(Builder, M);


				if ( auto *LI = dyn_cast<LoadInst>( &(*BI) ) ){

					unsigned size = M.getDataLayout().getTypeStoreSize(LI->getType());
                    Builder.CreateCall(recordLoadFunc, {tid, ConstantInt::get(Type::getInt32Ty(Ctx), size)});

				}
				else if (auto *SI = dyn_cast<StoreInst>( &(*BI) )){

					unsigned size = M.getDataLayout().getTypeStoreSize(SI->getType());
                    Builder.CreateCall(recordLoadFunc, {tid, ConstantInt::get(Type::getInt32Ty(Ctx), size)});
				}
				else {
					continue; // TODO: Return Instr
				}
			}
	...
	struct host_pass : public ModulePass{
        static char ID;   
        host_pass() : ModulePass(ID) {}
